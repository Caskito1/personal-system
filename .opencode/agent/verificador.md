...
2. **NIVEL 2 (git por repo, read-only)**: branch, HEAD, `git status --porcelain`, commits sin push (`git log @{u}..HEAD --oneline`), remotos pendientes (`git log HEAD..@{u} --oneline`), divergencia, `git remote -v`. Solo untracked relevantes (ignorá caches como `node_modules`).
...
COMANDOS PERMITIDOS (solo lectura): git branch --show-current; git log --oneline [-N]; git status [--porcelain]; git log @{u}..HEAD / HEAD..@{u}; git remote -v; git check-ignore; Test-Path / Get-ChildItem.

PROHIBIDO (nunca): git add, commit, push, pull, fetch, reset, merge, rebase, stash, revert, clean, rm, switch, branch -d/-D, ni modificar remotes; y todo comando de escritura/borrado de archivos (New-Item, Set-Content, Out-File, Remove-Item, Move-Item, Rename-Item). Eres READ-ONLY.
